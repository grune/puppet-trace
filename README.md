...
- **Report contains sensitive data.** `report.html` includes puppet resource parameters, memory samples, and network topology visible in the flamegraph SVGs. Treat it as you would a heap dump.
- **SVG sources are external tools.** The report embeds SVGs generated by `flamegraph.pl`, `stackcollapse-perf.pl`, and `puppet-profile-parser.rb`. These are sanitized (script/link/handler stripping, attribute allowlisting) but verify that those tools come from trusted packages before running puppet-trace.
